...
import { useCollectionDataOnce, useDocumentDataOnce, useCollection } from 'react-firebase-hooks/firestore'
import { getFirestore, query, collection, doc, limit } from 'firebase/firestore';
...
const Main = forwardRef(({filters, data, type}, ref) => {
	const [ mount, setMount ] = useState()
	...
	async function handlePage (e) {
		...
		let newData = await (await fetch(`/api/collections/${type}/${e.currentTarget.value}`)).json()
	...
	return (
		<div className={styles.wrapper}>
			...
			<div className={styles["paginationContainer"]}>
				...
				{
					data ? Array(Math.floor((data['meta']['n_items'] - 9) / 9)).fill(0).map((item, index) => 
